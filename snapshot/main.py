...
def run_analyze(cache: SQLiteCache, limit: int = 0, export: bool = False):
    ...
    print(
        f"  - 全量会话总数:           {m['total_sessions']} 场 (提问总字数: {m['total_user_chars']:,} 字符)"
    )

    print("\n  ⏱️ [会话生命周期与心智时长 (分钟)]:")
    print(
        f"    • 全体时长中位数 (P50):  {d_stats['median']} min  |  算术均值 (Mean): {d_stats['mean']} min (受离群长尾拉动)"
    )
    print(
        f"    • 多轮会话中位数 (P50):  {md_stats['median']} min  |  多轮均值 (Mean): {md_stats['mean']} min"
    )
    print(
        f"    • 长尾攻坚分位数:       P75: {d_stats['p75']} min  |  P90: {d_stats['p90']} min  |  Max: {d_stats['max']} min"
    )
    print("    • 时长心智梯队分布:")
    ...
    print("\n  💬 [交互深度与思维摩擦力]:")
    print(
        f"    • 对话轮次 (Turns):     中位数: {t_stats['median']} 轮  |  均值: {t_stats['mean']} 轮  |  P90: {t_stats['p90']} 轮"
    )
    print(
        f"    • 深度攻坚会话 (≥5轮):   {t_stats['deep_count']} 场 (占比 {t_stats['deep_ratio']})"
    )
    print(
        f"    • 思维摩擦力 (重试/分叉): {f_stats['branch_sessions']} 场 (占比 {f_stats['branch_ratio']}, 累计分支重试: {f_stats['total_retries']} 次)"
    )

    print("\n  🧠 [计算能耗与思考链 (Tokens)]:")
    print(
        f"    • 单场消耗中位数 (P50):  {int(tok_stats['median']):,} Tokens  |  均值: {int(tok_stats['mean']):,} Tokens"
    )
    print(f"    • 高负荷消耗 (P90):      {int(tok_stats['p90']):,} Tokens")
    print(f"    • 总消耗能耗:           {tok_stats['total']:,} Tokens")
    print(
        f"    • 思考链消耗 (Thinking): {tok_stats['total_thought']:,} Tokens (占总能耗: {tok_stats['thought_ratio']})"
    )
...
def cmd_fetch(args):
    ...
    total_valid, hits, downloaded = fetch_remote_files(
        client=client, cache=cache, limit=args.limit, all_files=args.all
    )
    print(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser_fetch = subparsers.add_parser(
        "fetch", help="[网络] 增量拉取云端最近修改的文件写入本地 SQLite"
    )
    parser_fetch.add_argument(
        "-n", "--limit", type=int, default=50, help="远程扫描拉取数量限制 (默认: 50)"
    )
    parser_fetch.add_argument(
        "--all", action="store_true", help="全量扫描云盘 (忽略 -n)"
    )

    # 2. analyze 子命令
    parser_analyze = subparsers.add_parser(
        "analyze", help="[离线] 纯离线分析本地 SQLite 缓存的历史会话"
    )
    parser_analyze.add_argument(
        "-n", "--limit", type=int, default=0, help="分析会话上限 (默认 0 表示全量分析)"
    )
    parser_analyze.add_argument(
        "--export",
        action="store_true",
        help="显式导出 CSV 与 JSONL 分析文件 (默认不导出)",
    )

    # 3. pull 子命令
    parser_pull = subparsers.add_parser(
        "pull", help="[组合] 增量拉取最近文件更新本地库，随后执行全量分析"
    )
    parser_pull.add_argument(
        "-n", "--limit", type=int, default=50, help="增量拉取数量限制 (默认: 50)"
    )
    parser_pull.add_argument("--all", action="store_true", help="全量扫描云盘并分析")
    parser_pull.add_argument(
        "--export",
        action="store_true",
        help="分析完成后显式导出 CSV 与 JSONL 文件 (默认不导出)",
    )

    # 4. serve 子命令
    parser_serve = subparsers.add_parser(
        "serve", help="[服务] 启动 FastAPI 本地数据后端服务"
    )
    parser_serve.add_argument(
        "--host", type=str, default="127.0.0.1", help="绑定监听地址 (默认: 127.0.0.1)"
    )
    parser_serve.add_argument(
...
def cmd_serve(args):
    ...
    print("=" * 60)
    uvicorn.run(
        "src.server.app:app", host=args.host, port=args.port, reload=args.reload
    )
...
def main():
    ...
    if args.command == "fetch":
        ...
        print(
            "   可用子命令: `fetch` (仅同步), `analyze` (仅本地分析), `pull` (同步并分析), `serve` (启动API服务)"
        )
