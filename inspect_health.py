...
def main():
    ...
    print(
        f"  - 碎片空间 (Freelist):       {format_bytes(free_bytes)} (空闲占比: {frag_ratio:.1f}%)"
    )
    ...
    print(
        f"  - 全文倒排索引数 (session_fts):   {format_num(fts_count) if fts_count is not None else '未创建'} 场"
    )
    ...
    if diff == 0:
        ...
        print(
            f"  ⚠️  索引状态: 存在 {abs(diff)} 条未同步记录，建议运行 python reindex_cli.py 重建"
        )
    ...
    print(
        f"{'指标项':<18} | {'Min':>8} | {'P10':>8} | {'P50 (中位)':>10} | {'P75':>8} | {'P90':>8} | {'P99':>9} | {'Max':>10}"
    )
    print("-" * 92)
    print(
        f"{'对话轮次 (Turns)':<18} | {q_turns['min']:>8.0f} | {q_turns['p10']:>8.0f} | {q_turns['p50']:>10.0f} | {q_turns['p75']:>8.0f} | {q_turns['p90']:>8.0f} | {q_turns['p99']:>9.0f} | {q_turns['max']:>10.0f}"
    )
    print(
        f"{'总 Token 消耗':<18} | {format_num(q_tokens['min']):>8} | {format_num(q_tokens['p10']):>8} | {format_num(q_tokens['p50']):>10} | {format_num(q_tokens['p75']):>8} | {format_num(q_tokens['p90']):>8} | {format_num(q_tokens['p99']):>9} | {format_num(q_tokens['max']):>10}"
    )
    print(
        f"{'思考链 (Thinking)':<18} | {format_num(q_thought['min']):>8} | {format_num(q_thought['p10']):>8} | {format_num(q_thought['p50']):>10} | {format_num(q_thought['p75']):>8} | {format_num(q_thought['p90']):>8} | {format_num(q_thought['p99']):>9} | {format_num(q_thought['max']):>10}"
    )
    print(
        f"{'持续时长 (分钟)':<18} | {q_dur['min']:>8.1f} | {q_dur['p10']:>8.1f} | {q_dur['p50']:>10.1f} | {q_dur['p75']:>8.1f} | {q_dur['p90']:>8.1f} | {q_dur['p99']:>9.1f} | {q_dur['max']:>10.1f}"
    )
    print(
        f"{'用户输入 (字符数)':<18} | {format_num(q_chars['min']):>8} | {format_num(q_chars['p10']):>8} | {format_num(q_chars['p50']):>10} | {format_num(q_chars['p75']):>8} | {format_num(q_chars['p90']):>8} | {format_num(q_chars['p99']):>9} | {format_num(q_chars['max']):>10}"
    )
    ...
    print(
        f"    - 轻量快问 (< 8k tokens):        {tok_under_8k:>5} 场 ({tok_under_8k / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 标准推进 (8k ~ 32k tokens):    {tok_8k_32k:>5} 场 ({tok_8k_32k / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 深度攻坚 (32k ~ 128k tokens):  {tok_32k_128k:>5} 场 ({tok_32k_128k / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 巨型长文 (≥ 128k tokens):      {tok_over_128k:>5} 场 ({tok_over_128k / len(rows) * 100:>5.1f}%)"
    )
    ...
    print(
        f"    - 单轮快问 (1 轮结束):          {single_turn:>5} 场 ({single_turn / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 轻量追问 (2 ~ 4 轮):           {light_turn:>5} 场 ({light_turn / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 深度讨论 (5 ~ 15 轮):          {deep_turn:>5} 场 ({deep_turn / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 长线马拉松 (> 15 轮):          {epic_turn:>5} 场 ({epic_turn / len(rows) * 100:>5.1f}%)"
    )
    ...
    thought_ratio = (
        (sum_thought_tokens / sum_total_tokens * 100) if sum_total_tokens > 0 else 0
    )

    print("\n  🔹 假设 C: 思考链 (Thinking) 渗透率与占比:")
    print(
        f"    - 触发思考链的会话数:            {thinking_sessions:>5} 场 ({thinking_sessions / len(rows) * 100:>5.1f}%)"
    )
    print(
        f"    - 思考链消耗占总 Token 比例:     {thought_ratio:>5.2f}% ({format_num(sum_thought_tokens)} / {format_num(sum_total_tokens)})"
    )
    ...
    print(
        f"    - 发生分叉/编辑重试的会话:       {branch_sessions:>5} 场 ({branch_sessions / len(rows) * 100:>5.1f}%)"
    )
    ...
    for rank, row in enumerate(top_large_files, start=1):
        # 关联标题
        cursor.execute(
            "SELECT name FROM session_index WHERE file_id = ?;", (row["file_id"],)
        )
        srow = cursor.fetchone()
        name = srow["name"] if srow else "未知会话"
        print(
            f"    #{rank}. [{format_bytes(row['byte_len']):>9}] ID: {row['file_id']} - {name[:36]}"
        )
    ...
    for rank, row in enumerate(top_tok, start=1):
        print(
            f"    #{rank}. [{format_num(row['total_tokens']):>9} tok] (思考: {format_num(row['thought_tokens'])}) ID: {row['file_id']} - {row['name'][:34]}"
        )
    ...
    for rank, row in enumerate(top_turns, start=1):
        print(
            f"    #{rank}. [{row['turn_count']:>4} 轮 | {row['duration_human']:>7}] ID: {row['file_id']} - {row['name'][:42]}"
        )
    ...
    print(
        f"  1. 绝大多数日常会话在 {p50_tok:.0f} Token 左右，但长尾 P90 跃升至 {p90_tok:.0f} Token。"
    )
    print(
        f"  2. 单轮即走的会话占比达 {single_turn / len(rows) * 100:.1f}%，列表虚拟滚动无需为默认项保留深层展开负担。"
    )
    if top_large_files and top_large_files[0]["byte_len"] > 10 * 1024 * 1024:
        print(
            f"  3. 存在体积超 10MB 的怪兽会话 ({format_bytes(top_large_files[0]['byte_len'])})，详情面板必须继续使用按需惰性加载。"
        )
    print("=" * 70)
